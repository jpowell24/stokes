...
const char *path1="../data_files/spherediffusion3.csv";
...
double calculate_diffusion(double time_Duration, double delta_T, int trials){
    ...
    for(int counter = 0; counter < trials; counter++){ // this is used to run many simulations, for N particles 
        ...
        for(double i = 0; i < time_Duration; i += delta_T){
            ...
            double noise_x = distribution(generator); // generate noise in x
            double noise_y = distribution(generator); // generate noise in y
            ...
            delta_X = sqrt(2 * D_t * delta_T) * noise_x; // solve change in x
            delta_Y = sqrt(2 * D_t * delta_T) * noise_y; // solve change in y

            temp_X = temp_X + delta_X; // sets new coordinates
            temp_Y = temp_Y + delta_Y;

            // cout << noise_x << " , " << noise_y << endl;
...
int main(void) {
    ...
    calculate_diffusion(10, 0.01, 1000); // enter time in ms, enter delta_T in ms, enter delta_Position in microns
    cout << "End" << endl;
}
